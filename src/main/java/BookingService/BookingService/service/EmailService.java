...
public class EmailService {
    ...
    public void sendSimpleMessage(MailBody mailBody) {
        try {
            ...
            // Lấy OTP từ mailBody.text() và kiểm tra an toàn
            String otp = extractOtpFromText(mailBody.text());
            String otpDisplay = (otp != null && !otp.equals("N/A") && !otp.trim().isEmpty()) ? otp : "Không tìm thấy OTP";

            // Tạo nội dung HTML cho email OTP với giao diện đẹp
            String htmlContent = "<!DOCTYPE html>" +
                    "<html lang=\"vi\">" +
                    "<head>" +
                    "<meta charset=\"UTF-8\">" +
                    "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">" +
                    "<title>Beautya - Xác Minh OTP</title>" +
                    "<style>" +
                    "/* Reset CSS */" +
                    ...
                    ".otp-box { background: linear-gradient(90deg, #f8e1e1 0%, #fcebeb 100%); padding: 20px; border-radius: 8px; text-align: center; margin: 20px 0; }" +
                    ".otp-code { font-size: 28px; font-weight: 700; color: #d47a8c; letter-spacing: 5px; }" +
                    ".warning { font-size: 14px; color: #777; margin-top: 10px; }" +
                    ...
                    "<div class=\"header\">" +
                    "<h1>Beautya</h1>" +
                    "<p>Xác minh OTP của bạn</p>" +
                    "</div>" +
                    "<div class=\"content\">" +
                    "<div class=\"greeting\">Xin chào!</div>" +
                    "<div class=\"text-content\">" +
                    "<p>Chúng tôi nhận được yêu cầu đặt lại mật khẩu từ bạn. Dưới đây là mã OTP của bạn:</p>" +
                    "<div class=\"otp-box\">" +
                    "<div class=\"otp-code\">" + otpDisplay + "</div>" +
                    "<p class=\"warning\">Mã OTP này có hiệu lực trong 70 giây. Vui lòng không chia sẻ mã này với bất kỳ ai.</p>" +
                    "</div>" +
                    ...
                    "</div>" +
                    "<div class=\"footer\">" +
                    "© 2025 Beautya Beauty. All rights reserved.<br>" +
    ...
    // Hàm để trích xuất OTP từ text
    private String extractOtpFromText(String text) {
        String[] parts = text.split(": ");
        if (parts.length > 1) {
            return parts[1].trim(); // Lấy phần OTP sau dấu ": "
        }
        return "N/A"; // Trả về giá trị mặc định nếu không tìm thấy OTP
    }
}
