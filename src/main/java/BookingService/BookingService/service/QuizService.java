...
//
//
//    // Phương thức sửa đổi để lấy câu hỏi và đáp án
...
////                                answerData.put("score", answer.getScore()); // Tùy chọn, có thể bỏ
//                                return answerData;
//                            })
...
public class QuizService {
    ...
    public Map<String, Object> processQuiz(Map<Long, Long> selectedAnswers) {
        ...
        for (Map.Entry<Long, Long> entry : selectedAnswers.entrySet()) {
            ...
            // Cộng điểm vào loại da tương ứng
            skinTypeScores.put(skinType, skinTypeScores.get(skinType) + score);

            // Lưu thông tin câu hỏi và đáp án đã chọn
        ...
        Map<String, Object> result = new HashMap<>();
        result.put("userResponses", userResponses);           // Danh sách câu hỏi, đáp án, điểm
        result.put("skinTypeScores", skinTypeScores);         // Tổng điểm từng loại da
        result.put("detectedSkinType", detectedSkinType);     // Loại da kết luận
        result.put("recommendedServices", recommendedServices); // Dịch vụ gợi ý
